...
def _make_static(
    ...
    return _build_static_features(
        route_id, origin_var, dest_var, demand, origin_cong, dest_cong, origin_sample_count, dest_sample_count
    )
...
class TestBuildFeatureVector:
    ...
    def test_dtype_is_float32(self):
        vec = _build_feature_vector(1, _make_static(1, 0.0, 0.0))
...
class TestFeatureVectorWithMockedDB:
    def test_shape_end_to_end(self):
        ...
        static = _build_static_features(132, 1.2, 1.3, d_hist, c_hist, c_hist, 15, 15)
        vec = _build_feature_vector(24, static)
        assert vec.shape == (1, _N_FEATURES)
        assert vec.dtype == np.float32
